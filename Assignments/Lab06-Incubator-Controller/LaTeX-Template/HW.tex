...
\begin{latin}
\begin{tikzpicture}[
  blk/.style={draw, rounded corners, minimum width=3.0cm, minimum height=1.25cm, align=center, font=\scriptsize},
  >=Stealth
]
  \node[blk, fill=blue!8]   (mf) at (0,1.9)  {main\_fsm\\S1 / S2 / S3};
  \node[blk, fill=green!8]  (ff) at (0,-0.3) {fan\_fsm\\OUT / S1 / S2 / S3};
  \node[blk, fill=black!4]  (tg) at (-4.9,0.8) {tick\_gen\\\scriptsize once per minute};

  \draw[->, thick] (tg.east) -- ++(0.5,0) |- (mf.west) node[near end, above, font=\tiny] {tick};
  \draw[->, thick] (tg.east) -- ++(0.5,0) |- (ff.west);

  \draw[->, thick] (mf.south) -- (ff.north)
        node[midway, right, font=\tiny] {enable = (next state == S2)};

  \draw[->, thick] (mf.east) -- ++(1.5,0) node[right, font=\tiny] {heater, cooler};
  \draw[->, thick] (ff.east) -- ++(1.5,0) node[right, font=\tiny] {crs[3:0]};

  \draw[->, thick] (-4.9,2.6) -- (mf.west |- 0,2.6) node[midway, above, font=\tiny] {temp[7:0]};
  \draw[->, thick] (-2.0,2.6) |- (ff.north west);
\end{tikzpicture}
\end{latin}
\end{center}
...
این اشکال در اولین اجرای تست‌بنچ {دیده نشد}، چون مدل مرجع هم دقیقا همین برداشت اشتباه را داشت؛ در بخش تست توضیح داده شده که چطور پیدا شد.
...
{\small شکل ۱ - چرخه‌ی کامل کنترل دما (مقادیر \lr{temp} بر حسب درجه‌ی سانتیگراد)}
\end{center}
...
علاوه بر این، یک {ناوردا} (\lr{invariant}) به‌صورت پیوسته بررسی می‌شود:

\begin{latin}
\begin{lstlisting}[style=verilog,numbers=none]
// crs must never be non-zero while the cooler is off
always @(posedge Clk)
...
\begin{center}
\begin{tabular}{@{}p{4.0cm}p{7.0cm}r@{}}
\toprule
{تست} & {پوشش} & {نتیجه} \\
\midrule
\lr{tb\_incubator}      & مدل مرجع + جاروب کامل دما از چهار پیکربندی + ناوردای پیوسته & $345/345$ \\
\lr{tb\_incubator\_spec} & انطباق مستقیم با نمودارهای صورت آزمایش & $23/23$ \\
\bottomrule
\end{tabular}
\end{center}
...
درس دوم درباره‌ی روش تست بود. مقایسه با مدل مرجع تنها تا جایی معتبر است که مدل، برداشت درستی از صورت مسئله داشته باشد. نوشتن یک تست دوم مستقیما از روی مشخصات و بدون مدل مشترک، همان چیزی بود که اشکال را آشکار کرد.
