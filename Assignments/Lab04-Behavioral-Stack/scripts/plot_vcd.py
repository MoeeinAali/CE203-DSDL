...
def bus_label(bits: str, width: int) -> str:
    """Render a VCD binary string as hex, or x/z if any bit is unknown."""
    b = bits.lower()
    if any(c in "xz" for c in b):
        return b[0] * max(1, (width + 3) // 4)
    val = int(b, 2)
    return f"{val:0{max(1, (width + 3) // 4)}X}"


def draw_bus(ax, samples, t_end, width, color):
    ...
    for i, (t, v) in enumerate(samples):
        ...
        if t_next - t > 0.9:
            ax.text((t + t_next) / 2.0, 0.5, bus_label(v, width),
