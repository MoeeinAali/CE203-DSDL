...
#
# Naming: tests_<circuit>.vec  or  <name>.vec  (falls back to the project's
# single circuit when there is no exact/prefix match).
#
# Override with env:
#   JAVA=/path/to/java
#   LOGISIM_JAR=/path/to/logisim-evolution-*-all.jar
#   CIRC=waiting_room.circ
...
circuit_from_vec() {
  ...
  # Fallback: single circuit project -> use its only circuit name.
