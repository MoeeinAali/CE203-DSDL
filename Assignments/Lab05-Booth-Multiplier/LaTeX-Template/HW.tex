...
\subsubsection*{چرا پایه-۴؟ (شرط تسریع)}
...
\toprule
$Q_1$ & $Q_0$ & $Q_{-1}$ & {عملیات} & {تفسیر} \\
\midrule
0 & 0 & 0 & $A \mathrel{+}= 0$    & دنباله‌ی صفر \\
0 & 0 & 1 & $A \mathrel{+}= M$    & پایان دنباله‌ی یک \\
0 & 1 & 0 & $A \mathrel{+}= M$    & یک تکی \\
0 & 1 & 1 & $A \mathrel{+}= 2M$   & پایان دنباله‌ی یک \\
1 & 0 & 0 & $A \mathrel{-}= 2M$   & آغاز دنباله‌ی یک \\
1 & 0 & 1 & $A \mathrel{-}= M$    & صفر تکی \\
1 & 1 & 0 & $A \mathrel{-}= M$    & آغاز دنباله‌ی یک \\
1 & 1 & 1 & $A \mathrel{+}= 0$    & دنباله‌ی یک \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{latin}
\begin{tikzpicture}[
  reg/.style={draw, minimum height=0.85cm, align=center, font=\scriptsize},
  >=Stealth
]
  \node[reg, minimum width=3.0cm, fill=blue!8]   (A)  at (0,0)    {A\\(N+2 bits)};
  \node[reg, minimum width=2.6cm, fill=green!8, right=0pt of A] (Q) {Q\\(N bits)};
  \node[reg, minimum width=1.1cm, fill=orange!12, right=0pt of Q] (Q1) {Q$_{-1}$\\(1)};

  \draw[->, thick] ($(Q1.south)+(0,-0.1)$) -- ++(0,-0.55) -| ($(A.south)+(0,-0.1)$);
  \node[font=\scriptsize] at (0.2,-1.05) {arithmetic shift right by \textbf{2} each iteration};

  \draw[<-, thick] (A.north) -- ++(0,0.55)
      node[above, font=\scriptsize] {$A \mathrel{+}= \{0, \pm M, \pm 2M\}$};

  \draw[decorate, decoration={brace, amplitude=5pt}] ($(Q.south east)+(0,-1.35)$) -- ($(Q1.south east)+(0.05,-1.35)$);
  \node[font=\scriptsize, anchor=north] at ($(Q.south east)+(0.55,-1.55)$) {3-bit window};
\end{tikzpicture}
\end{latin}
\end{center}
...
این دقیقا همان اشکالی بود که در اولین اجرای تست ظاهر شد: تمام ۸۶ خطای اولیه مربوط به حالت $\mathrm{Multiplicand} = -128$ بود و پس از افزایش پهنای انباشتگر به $N+2$ برطرف شد.
...
{\small شکل ۱ - ضرب‌کننده‌ی بوث پایه-۴ با $N=8$}
\end{center}
...
\begin{center}
\begin{tabular}{@{}p{3.9cm}p{7.1cm}r@{}}
\toprule
{تست} & {پوشش} & {نتیجه} \\
\midrule
\lr{tb\_booth\_multiplier} & تمام ۶۵۵۳۶ جفت علامت‌دار ۸ بیتی + حالت‌های مرزی + ریست میانی & $65562/65562$ \\
\lr{tb\_booth\_param} & سه پهنای $N=4,8,16$ و اندازه‌گیری تعداد کلاک & $774/774$ \\
\bottomrule
\end{tabular}
\end{center}
...
در نهایت، مهم‌ترین درس عملی این آزمایش مسئله‌ی پهنای انباشتگر بود. حالت $M = -2^{N-1}$ در نگاه اول یک حالت مرزی بی‌اهمیت به نظر می‌رسد، اما دقیقا همان‌جایی است که $-2M$ سرریز می‌کند و طراحی بی‌سروصدا جواب اشتباه می‌دهد. اینکه تست به‌صورت \lr{exhaustive} نوشته شده بود باعث شد این اشکال بلافاصله دیده شود.
