...
def parse_vcd(path: Path):
    ...
    for m in re.finditer(
        ...
        if name not in names:
            names.append(name)
        # The same leaf name can appear at several widths in different
        # scopes (an entry's 1-bit `match` and the array's 16-bit `match`).
        # Keep the widest, so a bus is never drawn as a single line.
        id_to_width[name] = max(width, id_to_width.get(name, 0))
...
def bus_label(bits: str, width: int, fmt: str = "hex") -> str:
    """Render a VCD binary string per `fmt`: hex, dec, signed decimal or binary."""
    b = bits.lower()
